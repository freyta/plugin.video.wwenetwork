...
def add_stream(name, title, content_type, content_id, icon, fanart=None, info=None, video_info=None, audio_info=None, start_point=None):
    ...
    u=sys.argv[0]+"?mode="+str(mode)+"&content_id="+urllib.quote_plus(str(content_id))+"&content_name="+urllib.quote_plus(name.encode('ascii','ignore'))
    if start_point is not None:
        u = u+"&start_point="+urllib.quote_plus(start_point)
...
def addDir(name,mode,iconimage,fanart=None,content_id=None,year=None,sub_filter=None,season_id=None,path=None,info=None):
    ...
    u=sys.argv[0]+"?mode="+str(mode)+"&name="+urllib.quote_plus(name.encode('ascii','ignore'))+"&icon="+urllib.quote_plus(iconimage)
    if content_id is not None:
        u = u+"&content_id="+urllib.quote_plus(content_id)
    if year is not None:
        u = u+"&year="+urllib.quote_plus(year)
    if sub_filter is not None:
        u = u+"&sub_filter="+urllib.quote_plus(sub_filter)
    if season_id is not None:
        u = u+"&season_id="+urllib.quote_plus(season_id)
    if path is not None:
        u = u+"&path="+urllib.quote_plus(path)
